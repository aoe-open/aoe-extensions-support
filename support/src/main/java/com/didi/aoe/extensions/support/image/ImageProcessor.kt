 ...
 * @since 1.1.0
 */
class ImageProcessor private constructor(builder: Builder) : SequentialProcessor<TensorImage>(builder) {
    class Builder : SequentialProcessor.Builder<TensorImage>() {
        fun add(op: ImageOperator) = apply {
            super.add(op)
        }

        fun add(op: TensorOperator) = apply {
            super.add(TensorOperatorWrapper(op))
        }

        override fun build(): ImageProcessor {
